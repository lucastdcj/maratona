...
int orig[MAXM], dest[MAXM], empresa[MAXM], cor[MAXK];
...
int caminho() {
  ...
  while (!Q.empty()) {
    ...
    if (inY[u] == -1) {
      ...
      if (find(orig[Y[u]]) == find(dest[Y[u]])) {
	for (i = 0; i < nY; i++)
	...
	for (i = 0; i < nY; i++)
      ...
      for (i = 0; i < nX_Y; i++)
	if (!mark[X_Y[i]] && (empresa[u] == empresa[X_Y[i]] || !cor[empresa[u]])) {
	  p[X_Y[i]] = u;
	  mark[X_Y[i]] = 1;
	  if (inX2[X_Y[i]] != -1)
	    return X_Y[i];
...
int matroide() {
  ...
  while ((u = caminho()) != -1) {
    ...
    for (i = 0; i < n; i++) {comp[i] = i; rank[i] = 1;}
    for (i = 0; i < nY; i++) unite(find(orig[Y[i]]), find(dest[Y[i]])); 
    /* atualiza X2 e X1 */
    nX2 = nX1 = 0; memset(inX2, -1, sizeof(inX2)); memset(inX1, -1, sizeof(inX1));
    for (i = 0; i < m; i++) {
