...
s.erase(x);
v[x] = new_priority;
s.insert(x);  // priority change
v[x] = priority;
s.insert(x);  // insert element
int u = *s.begin();
s.erase(*s.begin());                           // remove element
...
int flow[2 * MMAX];
int cost[2 * MMAX];
int upper[2 * MMAX];  // upper bound capacity
int pi[NMAX];
int pot[NMAX];
int d[NMAX];
int n, m;
...
bool bellman_ford(int s) {
  ...
  rep(k, n) rep(u, n) rep(i, adj[u].size()) if (res_cap(adj[u][i]) > 0)
          pot[dest[adj[u][i]]] <
      ? = pot[u] + cost[adj[u][i]];
  rep(u, n) rep(i, adj[u].size()) if ((res_cap(adj[u][i]) > 0) &&
                                      (pot[dest[adj[u][i]]] >
                                       pot[u] + cost[adj[u][i]])) return false;
